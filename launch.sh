...
if [ "$mode" == "3" ]; then
	...
	prev_genes=$cur_dir/tmpResults/common_prev_genes
        ...
        # cut -f 1,2,16 $cur_dir/datasets/string_data.txt > $cur_dir/datasets/human_ppt_interactions.txt
        ...
        echo 'Creating ENSP-ENSG-gene name dictionary'
        #join by ENSP
        ...
        ###
	echo "Prepare confident networks (PPI combined score >= 950)"
        $CODE_PATH/string_ppi_selection.R -i $cur_dir/datasets/human_ppi_ensembl_genes.txt -c1 -c2 -k 950 -o $cur_dir/selected_PPIs/human_confident_PPI.txt
        echo "Clustering networks (rber_pots) and selection"
        confident_ppi=$cur_dir/selected_PPIs/human_confident_PPI.txt
        # TODO SEARCH FOR EXPRESSED GENES ($common_genes) IN HUMAN NETWORK
        $CODE_PATH/cdlib_clusterize.py -i $confident_ppi -o confident_results/rber_pots.txt -m rber_pots -s -S confident_results/rber_pots_stats.txt
        #0       ENSG00000173692
